...
<details>
  <br>
  <summary>위치기반 추천 서비스</summary>
  <p><strong>도입</strong> : 유저에게 위치상 가장 가까운 모임을 간편하게 추천하기 위해서 도입함 </p>
  <p><strong>문제상황</strong> : 각 모임들의 데이터를 불러온 뒤, 유저의 현재위치와 모임의 좌표값사이의 거리를 새로운 배열에 넣어주는 과정에서 맨 마지막으로 계산된 거리만 들어가는 현상 발생 (아마도 useState가 비동기적으로 처리되어서 그런것 같음 Batch현상?)</p>
  <p><strong>의견 결정</strong> : 리덕스를 사용하여, 전역변수에 각 계산된 거리를 넣어 배열을 만들고 스토어에서 데이터를 꺼내쓰는 방식으로 해결하였음 (dispatch는 동기적으로 처리되기때문에?)  </p>
</details> 
